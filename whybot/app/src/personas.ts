...
IMPORTANT: You must always include a comma between every object in the array, including before the last object. Never omit a comma between objects. Do not use periods or whitespace as separators. Do not add any text, explanation, or formatting outside the JSON array. Your output must be valid JSON, with no trailing commas and no extra text.

Correct example:
[{"question": "Why is the sky blue?", "score": 10}, {"question": "Why is the sky red in the evening?", "score": 7}]

Incorrect (invalid) example (missing comma!):
[{"question": "A?", "score": 8} {"question": "B?", "score": 7}]

Ensure the questions are relevant to the original question and are complementary/orthogonal to one another, not redundant.
Use the same language as the brief for your response. ONLY VALID JSON is allowed. Do not write any other text or explanation.
`;
    ...
    researcher: {
      ...
      Given that context, please provide a concise answer (in the same language as the question) to this follow up question: ${node.question}`;
        ...
        getPromptForQuestions: (node) => {
            return `You are a curious researcher that tries to uncover fundamental truths about a given question by repeatedly asking follow-up "why|what|how|when|where" follow-up questions. Here is the question you seek to answer: ${node.question}?
            You've already done some research on the topic, and have surfaced the following brief:
            ---
            brief: ${node.answer}
            ---
            Write 1-3 interesting follow-up questions on that brief.
            
            ${QUESTIONS_FORMAT_EXPLANATION}

            Write your questions in the same language as the brief. For example, if the brief is in English, write your questions in English.
            YOU MUST WRITE YOUR QUESTIONS IN THE SAME LANGUAGE AS THE BRIEF. ONLY VALID JSON IS ALLOWED.
