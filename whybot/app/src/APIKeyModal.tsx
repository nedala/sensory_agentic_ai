...
export function APIKeyModal({ open, onClose, apiKey, setApiKey }: APIKeyModalProps) {
    ...
    const validate = useCallback(
        async (key: string) => {
            ...
            } else if (!key.startsWith("sk-") || (key.length !== 51 && key.length !== 164)) {
                setStatus(KeyStatus.Error);
                setErrorMessage("API key should start with 'sk-' and be 51 or 164 characters long");
            } else {
                        ...
                        >
                            <Dialog.Panel className="relative max-w-[450px] transform overflow-hidden rounded-lg bg-gray-700 px-4 pb-4 text-left shadow-xl transition-all sm:my-8 sm:p-6">
                                ...
                                <div className="mt-2 text-xs text-gray-400">
                                    Your API key is sent directly to OpenAI from your browser; it never touches our
                                    servers!
                                </div>
                            </Dialog.Panel>
                        </Transition.Child>
